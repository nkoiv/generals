...
public class SoundManagerJavaFX implements SoundManager {
    ...
    private void initializeMediaPlayer() {
        try {
            ...
            Generals.logger.warning("Unable to create mediaplayer - media missing?");
            return;
        }
    ...
    public void loadMusicIntoPlaylist(String id, String filename) {
        try {
            ...
            Generals.logger.log(Level.WARNING, "Tried to [{0}] but failed", filename);
        }
    ...
    public void playMusic (String id) {
        ...
        if (this.musicPlaylist.containsKey(id)) {
            ...
            Generals.logger.info("Music "+id+" found, starting playback");
        } else {
            Generals.logger.log(Level.WARNING, "Tried to play music with id <{0}>, but it was not in the playlist", id);
            return;
        }
    ...
    public void stopMusic() {
        try {
            ...
            Generals.logger.warning("Stopping mediaplayer failed - player missing?");
        }
    ...
    public void toggleMusicMute() {
        try {
            ...
            Generals.logger.warning("Tried to toggle media player mute-status, but failed. Player missing?");
        }
    ...
    public void loadSoundEffects(String id, String filename) {
        try {
            ...
            Generals.logger.log(Level.WARNING, "Failed to load {0} - file missing?", filename);
        }
    }
    ...
    public void playSound(final String id) {
        if(!this.soundEffects.containsKey(id)) {
            Generals.logger.log(Level.WARNING, "Tried to play soundEffect with the id <{0}>, but it was not found.", id);
            return;
        }
